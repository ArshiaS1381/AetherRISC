...
[RiscvInstruction("SH3ADD", InstructionSet.Zba, RiscvEncodingType.R, 0x33, Funct3 = 6, Funct7 = 0x10,
    ...
    Description = "Shifts rs1 left by 3 and adds it to rs2. Useful for double-word array indexing.", 
...
public class Sh3addInstruction : RTypeInstruction
{
    ...
    public override void Execute(MachineState s, InstructionData d) =>
        s.Registers.Write(d.Rd, (s.Registers.Read(d.Rs1) << 3) + s.Registers.Read(d.Rs2));

    public override void Compute(MachineState state, ulong rs1Val, ulong rs2Val, PipelineBuffers buffers)
    {
        buffers.ExecuteMemory.AluResult = (rs1Val << 3) + rs2Val;
    }
}
