...
namespace AetherRISC.Core.Architecture.Hardware.ISA
{
    public abstract class Instruction : IInstruction
    {
        ...
        // NEW: Default to false for standard integer instructions
        public virtual bool IsFloatRegWrite => false;

        public abstract void Execute(MachineState state, InstructionData data);

        public virtual void Compute(MachineState state, ulong rs1Val, ulong rs2Val, PipelineMicroOp op)
        {
             throw new NotImplementedException($"Instruction {Mnemonic} does not implement Compute().");
        }
