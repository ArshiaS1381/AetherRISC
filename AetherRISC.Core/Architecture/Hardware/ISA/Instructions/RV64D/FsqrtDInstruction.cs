...
public class FsqrtDInstruction : RTypeInstruction
{
    ...
    public override void Execute(MachineState s, InstructionData d) => 
        s.FRegisters.WriteDouble(d.Rd, Math.Sqrt(s.FRegisters.ReadDouble(d.Rs1)));

    public override void Compute(MachineState state, ulong rs1Val, ulong rs2Val, PipelineBuffers buffers)
    {
        double v = state.FRegisters.ReadDouble(this.Rs1);
        buffers.ExecuteMemory.AluResult = BitConverter.DoubleToUInt64Bits(Math.Sqrt(v));
    }
}
