using AetherRISC.Core.Architecture;
using AetherRISC.Core.Architecture.ISA.Base;
using System;

namespace AetherRISC.Core.Architecture.ISA.Extensions.D
{
    public class FsqrtDInstruction : Instruction {
        public override string Mnemonic => "FsqrtD";
        public override int Rd { get; } public override int Rs1 { get; }
        public FsqrtDInstruction(int rd, int rs1) { Rd = rd; Rs1 = rs1; }
        
        public override void Execute(MachineState s, InstructionData d) {
            double v1 = s.FRegisters.ReadDouble(d.Rs1);
            double res = (double)(Math.Sqrt(v1));
            s.FRegisters.WriteDouble(d.Rd, res);
        }
    }
}
