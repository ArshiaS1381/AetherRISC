...
[RiscvInstruction("CTZ", InstructionSet.Zbb, RiscvEncodingType.ZbbUnary, 0x13, Funct3 = 1, Funct7 = 0x30, Rs2Sel = 1,
    ...
    Description = "Counts the number of 0 bits at the LSB end of the word.",
    Usage = "ctz rd, rs1")]
public class CtzInstruction : RTypeInstruction
{
    ...
    public override void Execute(MachineState s, InstructionData d)
    {
        ...
        ulong res = (s.Config.XLEN == 32) ? (uint)BitOperations.TrailingZeroCount((uint)val) : (ulong)BitOperations.TrailingZeroCount(val);
    ...
    public override void Compute(MachineState state, ulong rs1Val, ulong rs2Val, PipelineBuffers buffers)
    {
        ulong res = (state.Config.XLEN == 32) 
            ? (uint)BitOperations.TrailingZeroCount((uint)rs1Val) 
            : (ulong)BitOperations.TrailingZeroCount(rs1Val);
